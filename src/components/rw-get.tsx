...
import { IRWInternalComponentProps } from '..';
import { log } from '../utility/log-utility';
import * as hooks from '../utility/hooks-utility';

export interface IRWGetProps extends IRWInternalComponentProps {
    endpoint: string;
...
export function RWGet(props: IRWGetProps) {
    ...
    hooks.ue((props: IRWGetProps) => {
        ...
        (async () => {
            try {
                const { data, status } = await axios.get(props.endpoint, {
                    headers: props.header || {
                        "Content-Type": 'application/json'
                    },
                    signal: abortController.signal
                });
                log.message('RWGet', data);
                setContextData({ data, status: OperationStatus.Completed });
            }
            catch {
