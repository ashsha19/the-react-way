...
export interface IRWDeleteProps extends IRWInternalComponentProps {
    endpoint: string;
...
export function RWDelete(props: IRWDeleteProps) {
    ...
    hooks.ue((props: IRWDeleteProps) => {
        ...
        (async () => {
            try {
                const { data, status } = await axios.delete(props.endpoint,
                    {
